...
class Fun(commands.Cog):
    ...
    async def _anime(self, ctx, *, query):
        try:
            ...
            await ctx.send(
                embed=discord.Embed(description="No anime with that name was found.")
            )
            return
    ...
    async def _anime_news(self, ctx):
        ...
        embed = discord.Embed(
            title="Anime news",
            description="For the weeb army and in the honor of Atomic Baby",
            color=discord.Color.random(),
        )
        embed.add_field(
            name=news.titles[0],
            value=f"{news.description[0][:400]}... [Continue Reading]({news.links[0]})",
        )
        embed.add_field(
            name=news.titles[1],
            value=f"{news.description[1][:300]}... [Continue Reading]({news.links[1]})",
        )
        embed.add_field(
            name=news.titles[2],
            value=f"{news.description[2][:300]}... [Continue Reading]({news.links[2]})",
        )
        embed.add_field(
            name=news.titles[3],
            value=f"{news.description[3][:200]}... [Continue Reading]({news.links[3]})",
        )
        embed.add_field(
            name=news.titles[4],
            value=f"{news.description[4][:200]}... [Continue Reading]({news.links[4]})",
        )
        await ctx.send(embed=embed)
